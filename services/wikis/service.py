...
from models.wiki import WikiNew
...
@wikis_bp.get("/")
def get_wikis():
    return jsonify({"response": "GET /wikis/"}), 200
    # try:
    #     wikis_list = wikis.find()
    # except Exception as e:
    #     return jsonify({"error": "Error al consultar la base de datos"}), 404
    # try:
    #     wikis_json = json.loads(json_util.dumps(wikis_list))
...
# @wikis_router.route("/<id>", methods = ["GET"])
